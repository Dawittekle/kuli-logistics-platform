...
const tokenVerifier = new SupabaseTokenVerifier({ mode: env.supabaseJwtMode });
...
const resolveAuth = async (request, { required = true } = {}) => {
  ...
  const authUser = await tokenVerifier.verifyAuthorizationHeader(authorization);
  const currentUser = userRepository.findBySupabaseUserId(authUser.sub);
...
const routeRequest = async (request) => {
  ...
  if (method === 'GET' && path === '/api/v1/me') {
    const { authUser, currentUser } = await resolveAuth(request);
    assertActiveAccount(currentUser);

    return success(accountService.getCurrentUser(authUser));
  }
  ...
  if (method === 'PATCH' && path === '/api/v1/me') {
    ...
    const body = await parseJsonBody(request);
    return success(accountService.updateOwnProfile(authUser, body));
  }

  if (method === 'GET' && path === '/api/v1/admin/users') {
    const { currentUser } = await resolveAuth(request);
    assertActiveAccount(currentUser);
    assertRole(currentUser, [roles.admin]);

    return success(accountService.listUsers());
  }

  if (method === 'POST' && path === '/api/v1/admin/staff-users') {
    const { currentUser } = await resolveAuth(request);
    assertActiveAccount(currentUser);
    assertRole(currentUser, [roles.admin]);
    ...
    return success(
      accountService.provisionStaffUser({
        actor: currentUser,

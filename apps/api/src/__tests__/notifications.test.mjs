...
test('MongoNotificationRepository insertMany sends email when enabled', async () => {
  ...
  const logPath = '/tmp/kuli-notifications-test-sent-emails.log';
  const previousEmailLogPath = process.env.EMAIL_LOG_PATH;
  ...
  try {
    ...
    if (previousEmailLogPath === undefined) {
      delete process.env.EMAIL_LOG_PATH;
    } else {
      process.env.EMAIL_LOG_PATH = previousEmailLogPath;
    }
  }
});

...
- Production-ready authentication cleanup removes frontend demo account shortcuts. Mobile and admin now rely on real Supabase sessions, backend `/me` role routing, and provisioned staff profiles; `DEMO_AUTH_ENABLED` should remain `false` for final testing.

...
- Local demo auth is guarded by `DEMO_AUTH_ENABLED`/frontend demo flags, disabled by default in examples, blocked from production readiness, and paired with `npm run seed:fake-users` for fake client/truck-owner/staff/vehicle/ticket data.

...
- Real frontend implementation is tracked separately in [Frontend Progress](frontend_progress.md). Frontend Phase 0 has working Expo React Native and React/Vite foundations. Frontend Phase 1 now adds Supabase auth screens, backend `/me` role routing, account status states, staff-only admin login, assistant/admin guards, and an admin users table shell. Real client/owner Supabase credential smoke checks remain open until test accounts are supplied or created intentionally.

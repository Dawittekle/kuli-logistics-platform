...
- Local demo login preserves an existing profile's backend role by email; registering or using explicit demo role buttons is the path for intentionally changing role in local demo data.
- Mobile auth was refreshed on 2026-06-01 with a Stitch-style splash/login/register/verification surface. Email verification is now shown only when Supabase requires it, resend is manual with a local cooldown, and the local onboarding flag only controls whether the premium splash repeats on the same install.
